...
use super::{
    ...
    ActorSendTargetRef, ActorUploadKind, MAX_TIME_TRIGGER_DELAY_SECONDS,
...
pub(super) async fn run_actor_command(
    ...
    match command {
        ...
        ActorCommand::Upload {
            ...
            let record = match agenthub_db::insert_object_upload(&db, &upload).await {
                Ok(record) => record,
                Err(err) => {
                    ...
                    return Err(err).context("failed to publish upload metadata");
                }
            };
