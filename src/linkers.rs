...
pub(crate) const SLOCK_CONNECTOR_ID: &str = "slock";
pub(crate) const DEFAULT_SLOCK_LINKER_ID: &str = "slock-primary";
...
#[derive(Debug, Clone)]
pub(crate) struct SlockConfigInput {
    pub api_origin: String,
    pub client_id: String,
...
pub(crate) struct AppLinkerPrincipal {
    pub subject: String,
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub(crate) struct AppLinkerRecord {
...
#[derive(Debug, Clone, Serialize, PartialEq, Eq)]
pub(crate) struct SlockLinkAttempt {
    pub linker_id: String,
    pub state: String,
    pub expires_at: i64,
    pub return_url: String,
...
pub(crate) struct SlockExchangeCode {
    pub code: String,
    pub state: Option<String>,
}

#[derive(Clone)]
pub(crate) struct AppLinkerService {
    db: SqlitePool,
    http: reqwest::Client,
}
...
impl AppLinkerService {
    ...
    pub(crate) async fn get_slock_linker(&self) -> anyhow::Result<Option<AppLinkerRecord>> {
        self.ensure_schema().await?;
        let row = sqlx::query(
            r#"
            SELECT
    ...
    pub(crate) async fn upsert_slock_config(
        &self,
        user_id: &str,
    ...
    pub(crate) async fn create_slock_link_attempt(
        &self,
        user_id: &str,
    ...
    pub(crate) async fn exchange_slock_code(
        &self,
        user_id: Option<&str>,
    ...
    pub(crate) async fn list_slock_channels(&self) -> anyhow::Result<Value> {
        self.ensure_schema().await?;
        let record = self
            .get_slock_linker()
            .await?
    ...
    pub(crate) async fn list_slock_channel_messages(
        &self,
        channel_id: &str,
    ) -> anyhow::Result<Value> {
        self.ensure_schema().await?;
...
pub(crate) fn parse_slock_exchange_code(
    code: Option<&str>,
