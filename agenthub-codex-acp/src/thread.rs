...
impl<A: Auth> ThreadActor<A> {
    ...
    async fn handle_prompt(
        ...
        if let Some((name, rest)) = extract_slash_command(&items) {
            match name {
                "permission-demo" => {
                    self.handle_permission_demo(response_tx).await?;
    ...
    async fn handle_permission_demo(
        &mut self,
        response_tx: oneshot::Sender<Result<StopReason, Error>>,
    ) -> Result<(), Error> {
        let client = self.client.clone();
        tokio::task::spawn_local(async move {
            let call_id = ToolCallId::new(format!("permission-demo-{}", Uuid::new_v4()));
            let init_fields = ToolCallUpdateFields::new()
                .title("Permission demo".to_string())
                .kind(ToolKind::Read)
                .status(ToolCallStatus::Pending)
                .raw_input(json!({ "path": "/etc/hosts" }));
            client
                .send_tool_call_update(ToolCallUpdate::new(call_id.clone(), init_fields))
                .await;
            ...
            let req_fields = ToolCallUpdateFields::new()
                .title("Permission demo".to_string())
                .kind(ToolKind::Read)
                .status(ToolCallStatus::Pending);

            let response = client
                .request_permission(ToolCallUpdate::new(call_id.clone(), req_fields), options)
                .await;
            ...
            match response {
                Ok(reply) => match reply.outcome {
                    ...
                    _ => {
                        client.send_agent_text("Permission rejected.").await;
                        let fields = ToolCallUpdateFields::new()
                            .status(ToolCallStatus::Failed)
                            .raw_output(json!({ "result": "rejected" }));
                        client
                ...
                Err(err) => {
                    client
                        .send_agent_text(format!("Permission request failed: {err}"))
                        .await;
