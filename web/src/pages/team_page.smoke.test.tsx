...
describe("TeamPage smoke render", () => {
  ...
  it("falls back to the active channel lane when the routed task no longer exists", async () => {
    const buildTeam = (id: string, name: string) => ({
      id,
      name,
      description: "Mission",
    ...
    window.history.replaceState(
      {},
      "",
      "/workspace/teams/team-1?lens=channels&channel=review&task=task-missing"
    );
        ...
        root.render(
          <MantineProvider>
            <TeamPage
              ...
              developerMode={false}
              routeTeamId="team-1"
              routeSearch="?lens=channels&channel=review&task=task-missing"
            />
          </MantineProvider>
        );
        await Promise.resolve();
  ...
  it("keeps the shared thread active for the default channels lens", async () => {
    const buildTeam = (id: string, name: string) => ({
      id,
      name,
      description: "Mission",
    ...
    getTeamSharedThread.mockResolvedValue(
      buildSharedThreadDetail("team-1", "task-all", "run-all")
    );

    window.history.replaceState({}, "", "/workspace/teams/team-1?lens=channels");
    ...
    try {
      ...
      expect(lastOptions.selectedConversation?.id).toBe("task-all");
      expect(window.location.pathname).toBe("/workspace/teams/team-1");
      expect(window.location.search).toBe("?lens=channels");
      expect(getTeamTask).not.toHaveBeenCalled();
    } finally {
      act(() => {
        root.unmount();
      });
  ...
  it("falls back to the shared channels lane when the routed channel no longer exists", async () => {
    const buildTeam = (id: string, name: string) => ({
      id,
      name,
      description: "Mission",
