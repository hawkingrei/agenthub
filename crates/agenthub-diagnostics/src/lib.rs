...
pub mod agent_trace {
    ...
    impl AgentTraceRequest {
        pub fn normalize_limit(&self) -> i64 {
    ...
    pub struct AgentTraceLiveOverlay {
    ...
    #[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
    pub struct AgentTraceRuntimeSummary {
    ...
    #[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
    pub struct AgentTraceAvailability {
        pub status: String,
        pub note: String,
    ...
    #[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
    pub struct AgentTraceEventSummary {
    ...
    #[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
    pub struct AgentTracePermissionSummary {
    ...
    pub async fn collect_from_pool(
        db: &SqlitePool,
    ...
    pub fn render_human(report: &AgentTraceReport) -> String {
        let mut lines = vec![
        ...
        let mut recent = rows
            ...
            .map(|row| {
                let message = row
                    ...
                    .ok()
                    .map(|bytes| agenthub_agent_event_codec::decode_message_from_storage(&bytes))
                    .unwrap_or_default();
                event_from_row(
                    row.get("id"),
                    row.get("session_id"),
    ...
    mod tests {
        ...
        async fn agent_trace_decodes_compressed_acp_event_metadata() {
            ...
            let encoded = agenthub_agent_event_codec::encode_message_for_storage(
                &agenthub_agent_domain::OutputStream::Acp,
                &message,
            );
