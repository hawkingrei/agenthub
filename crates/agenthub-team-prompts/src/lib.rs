pub const DEFAULT_TEAM_LEADER_PROMPT: &str = concat!(
    ...
    "- Use `\"$AGENTHUB_ACTOR_CLI\" actor team-task-create` to create canonical Team tasks; a conversational claim like 'task opened' is not sufficient unless the task is visible through `actor team-tasks`.\n",
    ...
    "- Use `\"$AGENTHUB_ACTOR_CLI\" actor time-trigger-set/list/cancel` for deferred follow-ups or timed reminders that should come back as ACP messages later.\n",
...
pub const DEFAULT_TEAM_WORKER_PROMPT: &str = concat!(
    ...
    "- Use `\"$AGENTHUB_ACTOR_CLI\" actor time-trigger-set/list/cancel` for timed rechecks, reminders, or follow-ups that should wake you up later through ACP.\n",
    "- `agent_loop` is an operator-controlled idle watchdog: it is disabled by default, enabled externally per agent, and only injects a configured ACP reminder after silence. Treat loop prompts as follow-up nudges, not as new human intent.\n",
    "- Do not assume you may enable or retune `agent_loop` yourself unless a human/operator explicitly asks for it.\n",
    ...
    "- Do not call `actor team-task-create` or `actor team-task-update`; canonical Team task creation and lifecycle updates remain leader-owned.\n",
    "- Treat `spec.members[]` as static baseline; when live roster and static spec differ, trust `actor team-members` for current execution decisions.\n",
