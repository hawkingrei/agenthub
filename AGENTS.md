...
- Code comment policy: when logic depends on non-obvious intent, compatibility behavior, hidden invariants, path/session/status derivation, or other assumptions that could be misread in a later change, add a short code comment explaining why the branch exists and what boundary it protects. Prefer comments that prevent future behavioral regressions over comments that restate syntax.
- Agent context/file-boundary policy: there is no hard single-file LOC cap, but code should be organized for token-efficient navigation; prefer cohesive files plus thin routing/index files, and split files when mixed responsibilities or sustained work would require reading large unrelated sections (rough warning threshold: ~800-1000 LOC or multi-domain ownership). When a file grows beyond that threshold or accumulates multiple ownership domains, agents should proactively split it into clearer modules instead of continuing to pile changes into the same file.
...
- Frontend implementation policy:
  ...
  - Fix obvious local code issues discovered during the active edit in the same change
