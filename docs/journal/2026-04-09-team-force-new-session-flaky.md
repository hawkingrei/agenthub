...
The restart test only needs a deterministic long-lived process so it can verify `running -> forced_restart -> new_session_id`. It does not need full actor CLI behavior, so the test now explicitly swaps the affected member agents onto a stable `/bin/sh -c 'exec /bin/sleep 30'` command before starting the Team runtime, resets them to `use_existing`, and stops the launched agents before the test returns.
