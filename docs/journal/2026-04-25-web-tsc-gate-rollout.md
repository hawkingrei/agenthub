...
Pay down the historical TypeScript backlog in `web/` until `pnpm exec tsc --noEmit` is clean, then wire the same check into CI so frontend type regressions fail the main `Web` workflow.
