...
- Use Team Forge-created Agent records as team members (leader/worker select boxes bind to
  forged Agent IDs only), and show selected member workdir so work path ownership is explicit.
...
- Team member `member_id` in generated spec maps to forged Agent `id`; workdir/worktree still comes from Agent config and is not duplicated in Team spec.
...
3. Create a team with multiple workers and verify member selection uses Team Forge-created agents and displays workdir.
