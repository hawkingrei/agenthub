...
- [ ] Verify Team Forge `Generate prompts & skills` (`POST /api/teams/assist`) fills leader/worker prompt + skills from brief, and `spec.workspace.root` provisioning creates `<root>/.agent` and `<root>/worktrees/*` with agent workdir rebinding (see `docs/features/2026-02-16-team-forge-ai-assist-workspace-layout.md`).
