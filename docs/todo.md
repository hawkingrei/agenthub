...
- [x] Verify new Team skill `team-deliberation-rules` is included in Team default leader/worker skills and appended by `scripts/setup_team_skills.sh` into `~/.agenthub/skills.json` without dropping existing skill entries (see `docs/features/2026-02-18-team-deliberation-rules-skill.md`).
- [x] Verify Team create flow restricts leader/worker selection to agents forged in the current Team create session (no selection from pre-existing Agent inventory), and that forge candidate pool resets on modal reopen (see `docs/features/2026-02-18-team-forge-only-member-selection.md`).
- [x] Verify Team spec enforces explicit member roles (`spec.members[].role` required, only `leader|worker`) across create-team API, router contract tests, and Team run lifecycle flows (see `docs/features/2026-02-18-team-member-role-required.md`).
- [x] Verify `/api/agents` hides member agents that are currently executing Team steps (`team_steps.status in working/input_required`) while keeping non-Team agents visible (see `docs/features/2026-02-18-team-agent-list-runtime-isolation.md`).
- [x] Verify Agent/Team shared status design language: `StatusBadge` tone mapping (`active/inactive/warning/danger/neutral`) renders consistently across Agents list, output header, and Teams run/member status chips, including active-row contrast in `/teams` (see `docs/features/2026-02-18-agent-team-shared-status-design-language.md`).
...
- [x] Verify send-input reliability under active SSE connections: after posting input, frontend boost polling can recover missed events, and backend closes lagged SSE streams to force reconnect + replay (see `docs/features/2026-02-18-send-input-sse-lagged-recovery.md`).
- [x] Verify web UI shows explicit unexpected-exit alert when active agent status transitions from `running/idle` to `failed/exited`, while normal `stopped` remains silent (see `docs/features/2026-02-18-agent-unexpected-exit-alert.md`).
- [x] Verify ACP-native actor mailbox tools (`actor_send`/`actor_inbox`/`actor_ack`) can run agent collaboration flows in local mode without requiring `AGENTHUB_ACTOR_CLI` (see `docs/features/2026-02-18-acp-actor-mailbox-native-tools.md`).
...
- [x] Verify distributed actor transport (A2A) keeps mailbox semantics consistent with local mode for idempotency, bounded retries (no infinite retry), and dead-letter transitions (see `docs/features/2026-02-18-agent-actor-local-distributed-architecture.md`).
- [x] Verify actor mailbox API implementation matches documented contract for `actor_send`/`actor_inbox`/`actor_ack` request fields, state transitions, and HTTP error mapping (`400/401/403/404/409/410/422/429`) (see `docs/features/2026-02-18-agent-actor-local-distributed-architecture.md`).
- [x] Verify Teams mailbox REST endpoints remain wire-compatible after switching to `ActorMailboxService` adapter (`send`/`inbox`/`ack` response payloads and idempotency behavior) (see `docs/features/2026-02-18-agent-actor-local-distributed-architecture.md`).
- [x] Verify Team internal gRPC mailbox endpoints (`send_actor_message` / `list_actor_inbox` / `ack_actor_message`) remain wire-compatible after switching to `ActorMailboxService` adapter (status mapping + payload fields) (see `docs/features/2026-02-18-agent-actor-local-distributed-architecture.md`).
...
- [x] Verify latest codex ACP event types (`plan`, `available_commands`, `current_mode`, `run_status`) are persisted/rendered as ACP stream (not stderr/system) for ACP agents while non-ACP agents keep stderr classification unchanged (see `docs/features/2026-02-18-send-input-session-guard-and-acp-type-compat.md`).
...
- [x] Verify collapsed agents rail and expanded agent rows both surface pending ACP permissions with red indicators during active permission polling and agent switching (see `docs/features/2026-02-17-agents-interaction-polish.md`).
- [x] Verify agent switch output hydration no longer waits for a second fetch when resolving `latest` session ids, and terminal auto-follow stays at bottom during in-place output updates (see `docs/features/2026-02-17-agents-interaction-polish.md`).
- [x] Verify mobile text input no longer triggers white-screen shell collapse when keyboard transitions cause transient `visualViewport` anomalies (`0/1` or non-finite dimensions) (see `docs/features/2026-02-17-mobile-input-white-screen-guard.md`).
- [x] Verify mobile/tablet workspace keeps input dock pinned to bottom of output panel under agent list collapsed/expanded states and keyboard open/close transitions (see `docs/features/2026-02-17-mobile-input-dock-bottom-alignment.md`).
- [ ] Verify ACP filesystem guard resolves relative `apply_patch` paths against session root (not process CWD) and rejects traversal outside root, matching upstream codex-acp behavior (see `docs/features/2026-02-17-codex-acp-upstream-sync-path-ordering.md`).
- [ ] Verify ACP tool-call/debug JSON preserves key ordering for keychain/MCP-sensitive payloads by keeping `serde_json/preserve_order` enabled in `agenthub-codex-acp` (see `docs/features/2026-02-17-codex-acp-upstream-sync-path-ordering.md`).
- [ ] Verify ACP prompt state supports concurrent `exec_command` streams without losing active command state when begin/end events interleave by `call_id` (see `docs/features/2026-02-17-codex-acp-upstream-sync-thread-state.md`).
- [ ] Verify `/compact` and `/undo` run through unified `PromptState` handling after `TaskState` removal, including turn completion and undo status messaging (see `docs/features/2026-02-17-codex-acp-upstream-sync-thread-state.md`).
...
- [x] Verify Team Workbench UI (`/teams`) create/run/step/message interaction flow end-to-end in a real browser (see `docs/features/2026-02-16-team-workbench-ui.md`).
- [x] Verify Team creation quest modal (`Create Team` dialog stages: Mission Brief -> Leader Forge -> Recruit Workers -> Launch Team) works on desktop/mobile, including leader-only flow, manual JSON override path, and Team-Forge-only member selection with workdir visibility (see `docs/features/2026-02-16-team-create-quest-modal.md`).
- [x] Verify Team creation modal leader/worker model selectors stay in sync with Agent preset options (`Codex ACP`/`Gemini CLI`/`Kimi CLI`) and no longer require manual model text input (see `docs/features/2026-02-16-team-create-quest-modal.md`).
...
- [x] Verify Playwright Team UI E2E (`web/tests/e2e/team_page.e2e.ts`) stays green in CI and covers staged Team Forge creation with mocked API responses for leader/worker model selection and generated workflow steps (see `docs/features/2026-02-16-team-ui-e2e-playwright.md`).
- [x] Verify Team Forge stage guardrails (`Next Stage` blocking reason, stage jump lock, duplicate member prevention, `Resolve Duplicates`, `Auto Fill Party`) on desktop/mobile to ensure quest flow remains smooth (see `docs/features/2026-02-16-team-forge-quest-flow-ux-guardrails.md`).
- [x] Verify Team run snapshot API (`GET /api/teams/runs/:run_id/snapshot`) and `/teams` Overview/Mailbox/Member Console tabs stay consistent under active run updates (see `docs/features/2026-02-16-team-snapshot-member-console.md`).
...
- [x] Verify Team run list UI status filter + `Load More` paging controls keep active run selection stable while switching filters/teams (see `docs/features/2026-02-17-team-run-list-filter-pagination-controls.md`).
- [x] Verify Team run list uses server cursor (`before_created_at`) instead of offset-style paging and keeps per-team filter memory when switching teams (see `docs/features/2026-02-17-team-run-list-cursor-and-per-team-filter-memory.md`).
- [x] Verify Team output preview policy: before selecting a specific member, `/teams` `Events` and `Member Console` show only latest 5 run records; selecting a member restores full member history (see `docs/features/2026-02-17-team-output-preview-limit.md`).
- [x] Verify Team mailbox template workflow (`leader_task_assignment` / `clarification_request` / `worker_status` / `profile_patch_proposal`) can be applied from UI and remains aligned with backend default leader/worker prompts (see `docs/features/2026-02-17-team-mailbox-structured-templates.md`).
- [x] Verify extracted `TeamMailboxPanel` component preserves IM conversation behavior (member focus, unread, auto-follow, advanced controls) in Playwright (see `docs/features/2026-02-18-team-mailbox-panel-component-extraction.md`).
- [x] Verify extracted `TeamMemberConsolePanel` component preserves preview/member-history refresh behavior in Team Workbench (see `docs/features/2026-02-18-team-member-console-panel-component-extraction.md`).
- [x] Verify extracted `TeamSidebar` component preserves Team list selection and Team Forge launch behavior in full Team Playwright flow (see `docs/features/2026-02-19-team-sidebar-component-extraction.md`).
- [x] Verify extracted `TeamRunPanel` component preserves run filter/paging/selection and create-load controls in full Team Playwright flow (see `docs/features/2026-02-19-team-run-panel-component-extraction.md`).
- [x] Verify extracted `TeamOverviewPanel` component preserves snapshot refresh/member->mailbox jump behavior in Team Playwright flow (see `docs/features/2026-02-19-team-overview-events-panel-component-extraction.md`).
- [x] Verify extracted `TeamEventsPanel` component preserves event auto-refresh toggle and load-older behavior in Team Playwright flow (see `docs/features/2026-02-19-team-overview-events-panel-component-extraction.md`).
- [x] Verify extracted `TeamStepsPanel` component preserves step submit/action lifecycle controls in Team Playwright flow (see `docs/features/2026-02-19-team-steps-panel-component-extraction.md`).
- [x] Verify Team page UI reducer phase-1 (`tab`/`runLookupId`/`eventsAutoRefresh`) keeps Team flows stable in unit + Playwright checks (see `docs/features/2026-02-19-team-ui-state-reducer-phase1.md`).
- [x] Verify Team page UI reducer phase-2 (`runContextId`/`runInput` + step form/action controls) keeps Team flows stable in unit + Playwright checks (see `docs/features/2026-02-19-team-ui-state-reducer-phase2-run-step.md`).
- [x] Verify Team page UI reducer phase-3 (mailbox/member conversation state: inbox filters, chat draft, unread markers, selected member) keeps Team flows stable in unit + Playwright checks (see `docs/features/2026-02-19-team-ui-state-reducer-phase3-mailbox-member.md`).
- [x] Verify Team page UI reducer phase-4 (create-team wizard + agent forge form state) keeps Team create/run flows stable in unit + Playwright checks (see `docs/features/2026-02-19-team-ui-state-reducer-phase4-create-team.md`).
- [x] Verify Team delete flow (`DELETE /api/teams/:id` + `/teams` `Delete Team` action) removes team-owned runs/events/steps/mailbox records and keeps next-team selection stable in desktop/mobile browsers (see `docs/features/2026-02-17-team-delete-api-ui.md`).
...
- [x] Refactor `web/src/pages/team_page.tsx` into smaller feature-focused components (`TeamSidebar`, `RunPanel`, tab subviews) and a reducer-based state model for maintainability (phase-1: `TeamSidebar` + `TeamRunPanel` + `TeamOverviewPanel` + `TeamEventsPanel` + `TeamMailboxPanel` + `TeamMemberConsolePanel` + `TeamStepsPanel` extracted; reducer phase-1/2/3/4 now cover tab/run filters, run/step controls, mailbox/member conversation state, and create-team wizard/forge state) (see `docs/features/2026-02-16-team-workbench-ui.md`, `docs/features/2026-02-18-team-mailbox-panel-component-extraction.md`, `docs/features/2026-02-18-team-member-console-panel-component-extraction.md`, `docs/features/2026-02-19-team-sidebar-component-extraction.md`, `docs/features/2026-02-19-team-run-panel-component-extraction.md`, `docs/features/2026-02-19-team-overview-events-panel-component-extraction.md`, `docs/features/2026-02-19-team-steps-panel-component-extraction.md`, `docs/features/2026-02-19-team-ui-state-reducer-phase1.md`, `docs/features/2026-02-19-team-ui-state-reducer-phase2-run-step.md`, `docs/features/2026-02-19-team-ui-state-reducer-phase3-mailbox-member.md`, `docs/features/2026-02-19-team-ui-state-reducer-phase4-create-team.md`).
