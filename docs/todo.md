...
- [ ] `P1` Implement explicit Team adoption extensions after the copy-first rollout: the stable contract now defines stopped-only `move existing agent to Team`, opt-in workspace-content copy, and opt-in memory/context seeding as separate reviewable modes with runtime, ownership, history, provenance, and retry guardrails. Remaining work is typed API/UI implementation plus focused backend/web tests and browser evidence. Stable contract: [features/team-agent-adoption.md](features/team-agent-adoption.md); notes: [journal/2026-05-03-team-agent-adoption-contract.md](journal/2026-05-03-team-agent-adoption-contract.md).
