...
- [ ] `P1` Verify the trimmed `Release Prebuild` workflow after the Codex ACP runtime package split lands: run `29694176620` after `#893` still uploaded only `release-prebuild-{target}` artifacts and packaged only `agenthub` / `agenthub-acp`, but its logs still compiled package `agenthub-codex-acp v0.10.0` through `agenthub-acp-adapter`. The follow-up split renames that internal package to `agenthub-codex-acp-runtime` while keeping the compatibility binary name. Remaining evidence is the next `push` to `main` `Release Prebuild` run proving the logs compile `agenthub-codex-acp-runtime`, publish only the canonical `agenthub` / `agenthub-acp` archives and Linux `.deb` files, exercise the same Linux release feature paths, and catch release-only cross regressions before tags are cut. Record workflow run IDs in [journal/2026-04-20-release-vendored-openssl-and-partial-assets.md](journal/2026-04-20-release-vendored-openssl-and-partial-assets.md).
