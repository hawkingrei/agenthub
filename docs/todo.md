...
- [ ] Verify send-input reliability under active SSE connections: after posting input, frontend boost polling can recover missed events, and backend closes lagged SSE streams to force reconnect + replay (see `docs/features/2026-02-18-send-input-sse-lagged-recovery.md`).
- [ ] Verify web UI shows explicit unexpected-exit alert when active agent status transitions from `running/idle` to `failed/exited`, while normal `stopped` remains silent (see `docs/features/2026-02-18-agent-unexpected-exit-alert.md`).
