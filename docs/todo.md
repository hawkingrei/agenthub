...
- [ ] Verify Team run list uses server cursor (`before_created_at`) instead of offset-style paging and keeps per-team filter memory when switching teams (see `docs/features/2026-02-17-team-run-list-cursor-and-per-team-filter-memory.md`).
...
- [ ] Verify Team mailbox template workflow (`leader_task_assignment` / `clarification_request` / `worker_status` / `profile_patch_proposal`) can be applied from UI and remains aligned with backend default leader/worker prompts (see `docs/features/2026-02-17-team-mailbox-structured-templates.md`).
- [ ] Verify Team delete flow (`DELETE /api/teams/:id` + `/teams` `Delete Team` action) removes team-owned runs/events/steps/mailbox records and keeps next-team selection stable in desktop/mobile browsers (see `docs/features/2026-02-17-team-delete-api-ui.md`).
