...
- [ ] `P0` Channel/thread layout: `# all` and non-default channels route correctly, channel without an open thread uses the full center lane, opening a thread splits channel/thread without overlap, and compact viewport pane switching has no horizontal overflow. Record deployed notes and CI run IDs in the active Team workspace journal.
- [ ] `P0` Workspace search command: Search stays an independent command dialog, filters channels/tasks/agents locally, does not navigate to a normal `Search` content lens, and selecting a result updates the route and right-side content.
- [ ] `P1` Conversation and composer polish: message rows keep the wider Slock-style content lane, human/agent bubbles stay neutral, thread pane reads like the same chat system as the center lane, and channel/thread/ACP composers share one lightweight input language. Existing notes: `docs/journal/2026-04-24-team-conversation-slock-polish.md`.
- [ ] `P1` Team shell refinement and route chunk split: selector remains a slim chooser, Team/ACP shells stay thin and wide, read-state indicators remain unobtrusive, and deployed `index.html` does not preload route chunks that should be lazy. Existing notes: `docs/journal/2026-03-21-team-ui-shell-and-bundle-refinement.md`.
- [ ] `P1` Agents workbench lazy-split LCP: deployed `agents` route should ship a small primary shell chunk while OutputBody/InputDock/ACP workbench code loads lazily; evaluate `AcpDebug` as a follow-up split if it still dominates. Existing notes: `docs/journal/2026-04-06-agents-lcp-workbench-split.md`.
- [ ] `P2` PWA installability: deployed manifest/service-worker remain installable without stale shell caching; HTML shell routes, `sw.js`, and `manifest.webmanifest` stay `no-cache`, while `/assets/*` remains immutable. Existing notes: `docs/journal/2026-04-03-pwa-install-and-team-permission-card-collapse.md`.

## Team Workspace Architecture

- [ ] `P0` Continue the remaining `web/src/app.tsx` and Team workspace composition cleanup after the initial route-container split: keep shrinking route-level state surfaces, extract the next stable Team view-model boundaries, and validate with focused route/team tests plus frontend typecheck/lint. Current checkpoint: `docs/journal/2026-05-08-team-route-container-cleanup.md`.
- [ ] `P0` Continue consolidating the web workbench onto shared UI primitives: migrate remaining Team panels, dock/toolbars, and row/list affordances onto `web/src/ui/primitives.tsx` so repeated Tailwind strings stop drifting independently. Current audit: `docs/journal/2026-05-02-web-tailwind-audit.md`.
- [ ] `P1` Phase 1/2 unified workspace shell follow-up: finish route parity, deeper shell reuse, and shared-lens cleanup without collapsing Team task-first semantics or turning `thread` into a top-level lens. Stable contract: `docs/features/workspace-unified-ia.md`; notes: `docs/journal/2026-04-18-workspace-shell-route-phase1.md` and `docs/journal/2026-04-18-workspace-shell-phases-1-3-convergence.md`.
- [ ] `P1` Extend first-run instance setup beyond `agenthub init`: add a reviewed web setup surface, then decide whether provider API base URLs / API keys should become first-class config instead of post-init operator guidance. Stable contract: `docs/features/instance-init-cli.md`.
- [ ] `P1` Evaluate explicit Team adoption extensions after the copy-first rollout: design stopped-only `move existing agent to Team`, opt-in workspace-content copy, and opt-in memory/context seeding as separate reviewable modes with runtime, ownership, and history guardrails. Stable contract: `docs/features/team-agent-adoption.md`.
- [ ] `P2` Frontend performance hardening for Team and ACP-heavy pages: reduce avoidable rerenders, keep long lists and live surfaces responsive, and evaluate virtualization/stick-to-bottom behavior for extremely long histories. Stable contracts: `docs/features/frontend-design.md` and `docs/features/acp-runtime.md`.
...
- [ ] `P1` Node startup boundaries: `server.role = "node"` boots internal gRPC only, skips main-only startup side effects, and fails fast when `server.node_id` is missing or `internal_grpc.enabled` is false. Existing notes: `docs/journal/2026-04-05-node-mode-startup-boundary.md`.
- [ ] `P1` Token-first Agent Node join: root `Agents` surfaces node bootstrap token/details, Admin join exposes token/link without QR onboarding, and user docs match the current `internal_grpc.bootstrap.token` contract. Existing notes: `docs/journal/2026-04-17-node-join-token-flow.md`.
...
- [ ] `P1` Verify deployed Pyroscope bootstrap: full configuration starts one process-wide profiler agent, partial configuration warns and keeps the service running, and shutdown stops the profiler cleanly. Stable contract: `docs/features/pyroscope-profiling.md`.
- [ ] `P1` Verify the TLS dependency floor refresh: workspace resolution keeps `rustls v0.23.37`, `aws-lc-rs v1.16.2`, and `aws-lc-sys v0.39.0`, and both `cargo check` plus `bazel build //...` stay green. Existing notes: `docs/journal/2026-03-26-rustls-aws-lc-version-floor.md`.
- [ ] `P1` Verify the Bazel CI linker baseline after installing `lld`: `bazel build //...` and `bazel test //...` stop emitting the Rust gold-linker deprecation warning, and workflow run IDs are recorded. Existing notes: `docs/journal/2026-03-24-bazel-ci-lld-linker.md`.
...
- [ ] `P2` Continue `docs/features` compaction wave 2: move stable conclusions out of residual Team/UI micro-journals into canonical feature specs and replace merged records with explicit supersession pointers where useful. See `docs/features/README.md`.
