...
- [ ] Verify Team internal gRPC mailbox endpoints (`send_actor_message` / `list_actor_inbox` / `ack_actor_message`) remain wire-compatible after switching to `ActorMailboxService` adapter (status mapping + payload fields) (see `docs/features/2026-02-18-agent-actor-local-distributed-architecture.md`).
