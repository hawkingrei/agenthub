...
- [ ] `P0` Channel/thread layout: `# all` and non-default channels route correctly, channel without an open thread uses the full center lane, opening a thread splits channel/thread without overlap, and compact viewport pane switching has no horizontal overflow. Local regression coverage and authenticated deployed browser evidence landed in [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md); record CI run IDs after PR.
- [ ] `P0` Workspace search command: Search stays an independent command dialog, filters channels/tasks/agents locally, does not navigate to a normal `Search` content lens, and selecting a result updates the route and right-side content. Local route/sidebar coverage and authenticated deployed browser evidence landed in [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md); record CI run IDs after PR.
...
- [ ] `P0` Continue the remaining `web/src/app.tsx` and Team workspace composition cleanup after the initial route-container split: keep shrinking route-level state surfaces, extract the next stable Team view-model boundaries, and validate with focused route/team tests plus frontend typecheck/lint. Current checkpoints: [journal/2026-05-08-team-route-container-cleanup.md](journal/2026-05-08-team-route-container-cleanup.md) and [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md).
- [ ] `P0` Continue consolidating the web workbench onto shared UI primitives: migrate remaining Team panels, dock/toolbars, and row/list affordances onto `web/src/ui/primitives.tsx` so repeated Tailwind strings stop drifting independently. Current audit: [journal/2026-05-02-web-tailwind-audit.md](journal/2026-05-02-web-tailwind-audit.md); latest slice: [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md).
...
- [ ] `P0` Codex ACP permission-deny recovery after PR #604: permission timeout or failed review delivery persists a concrete deny option, submits Codex `ReviewDecision::Denied` instead of `Abort`, avoids prompt abandonment/runtime restart, and surfaces any remaining no-event prompt via `agenthub doctor agent-trace`. Concrete deny fallback coverage landed in [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md); complete live Codex recovery validation.
...
- [ ] `P0` Verify the task-first Team model end to end: creating a Team task does not auto-create a run, `team_tasks.assigned_member_id` stays `NULL` until explicit ownership, and docs/skills keep `run` / `step` framed as execution and debug artifacts rather than primary collaboration state. Manager-level regression landed in [journal/2026-05-18-team-workspace-p0-matrix.md](journal/2026-05-18-team-workspace-p0-matrix.md); complete deployed end-to-end validation. Existing notes: [journal/2026-03-23-team-task-first-without-backend-orchestrator.md](journal/2026-03-23-team-task-first-without-backend-orchestrator.md).
